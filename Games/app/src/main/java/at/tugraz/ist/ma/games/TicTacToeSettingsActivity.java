...
public class TicTacToeSettingsActivity extends AppCompatActivity {
    ...
    private final int LAUNCH_TIC_TAC_TOE_GAME = 1;

    private final TicTacToeSettings ttt_settings_ = new TicTacToeSettings();
    @Override
    ...
    private void configBoardButtons(){
        ...
        play_btn.setOnClickListener(v -> handleGameStart());

        game_mode_tb.setOnCheckedChangeListener((buttonView, isChecked) -> {
            if (isChecked) {
        ...
        player_one_symbol_tb.setOnCheckedChangeListener((buttonView, isChecked) -> {
            if (isChecked) {
        ...
        player_two_symbol_tb.setOnCheckedChangeListener((buttonView, isChecked) -> {
            if (isChecked) {
                ttt_settings_.setStartingPlayer(TicTacToe.Tile.CIRCLE);
